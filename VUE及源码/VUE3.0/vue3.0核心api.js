...
let createReactive = (target, prop, value) => {
  target._dep = new Dep();
  // return new Proxy(target, { // Proxy可以监听对象(包括数组)上属性的新增,删除等操作
  //   get(target, prop) {
  //     dep.depend();
  ...
  return Object.defineProperty(target, prop, { // Object.defineProperty无法监听对象(包括数组)上属性的新增,删除等操作,但是对数组push,splice等方法做了重新封装
    get() {
      target._dep.depend();
      return value;
    },
...
let computed = fn => {
  ...
  return {
    get value() { // get语法将对象属性绑定到查询该属性时将被调用的函数。--MDN
      if (dirty) {
        value = runner();
        dirty = false;
      }
